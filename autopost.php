...
function img_get(){
 $data = file_get_contents('https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1') or return print("MayxDaily无法连接至壁纸服务器……");
...
for($i=0;$i<sizeof($rssfeed);$i++){//分解开始 
    ...
    $rss_str=""; 
    //打开rss地址，并读取
    $fp = fopen($rssfeed[$i],"r") or return print("MayxDaily无法连接至新闻服务器……");  
    while ( !feof($fp) ) { 
        $buff .= fgets($fp,4096); 
    } 
    //关闭文件打开 
